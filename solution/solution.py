...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "Score the answer for each rubric criterion from 0.0 to 1.0.\n"
            "Return JSON only, for example: {\"accuracy\": 0.8, \"clarity\": 0.7}.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_text}"
        )
        ...
        for criterion in rubric:
            value = scores.get(criterion, 0.5)
            try:
                numeric = float(value)
            except (TypeError, ValueError):
                numeric = 0.5
            normalized_scores[criterion] = max(0.0, min(1.0, numeric))
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            avg = sum(numeric_scores) / len(numeric_scores) if numeric_scores else None
            if avg is not None:
                if position in ("first", 0, 1, "A"):
                    first_scores.append(avg)
                elif position in ("second", 2, "B"):
                    second_scores.append(avg)
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = suggestions[index - 1] if index - 1 < len(suggestions) else "Review this failure and add a targeted fix"
            rows.append(
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append("Improve prompt clarity and intent routing so answers directly address the user question")
        if categories.get("incomplete", 0) > 0 or categories.get("Low_completeness", 0) > 0:
            suggestions.append("Add few-shot examples and answer checklists to improve completeness")
        if categories.get("off_topic", 0) > 0:
            suggestions.append("Add topic classification before generation to prevent off-topic responses")
        if categories.get("refusal", 0) > 0:
            suggestions.append("Tune safety rules to reduce unnecessary refusals on allowed questions")

        low_faithfulness = sum(1 for failure in failures if failure.faithfulness < 0.5)
        low_completeness = sum(1 for failure in failures if failure.completeness < 0.5)
        if low_faithfulness:
            suggestions.append("Improve retrieval quality with hybrid search, metadata filters, and reranking")
        if low_completeness:
            suggestions.append("Increase context window or chunk overlap so required evidence is not fragmented")

        defaults = [
            "Track recurring failures in the golden dataset and rerun regression tests before deploy",
            "Calibrate automated scores against human review for the hardest examples",
            "Log failure type, root cause, and fix owner for every benchmark failure",
        ]
